...
(function (DomType) {
    ...
    DomType["MoveFromRangeStart"] = "moveFromRangeStart";
    DomType["MoveFromRangeEnd"] = "moveFromRangeEnd";
    DomType["MoveToRangeStart"] = "moveToRangeStart";
    DomType["MoveToRangeEnd"] = "moveToRangeEnd";
    DomType["FormatChange"] = "formatChange";
...
class DocumentParser {
    ...
    parseMoveRangeMarker(node, type) {
        return {
            type: type,
            id: globalXmlParser.attr(node, "id"),
            name: globalXmlParser.attr(node, "name")
        };
    }
    parseAltChunk(node) {
        ...
        for (let el of globalXmlParser.elements(node)) {
            switch (el.localName) {
                ...
                case "commentReference":
                    result.children.push(new WmlCommentReference(globalXmlParser.attr(el, "id")));
                    break;
                case "oMath":
                case "oMathPara":
    ...
    parseFormatChange(node, context) {
        const formatDescriptions = [];
        for (const el of globalXmlParser.elements(node)) {
            switch (el.localName) {
                case "rPr":
                case "pPr":
                    for (const prop of globalXmlParser.elements(el)) {
                        const desc = this.getFormatDescription(prop);
                        if (desc)
                            formatDescriptions.push(desc);
                    }
                    break;
            }
        }
    ...
    getFormatDescription(prop) {
        switch (prop.localName) {
            case "b": return globalXmlParser.boolAttr(prop, "val", true) ? "Bold" : "Not bold";
            case "i": return globalXmlParser.boolAttr(prop, "val", true) ? "Italic" : "Not italic";
            case "u": return "Underline";
            case "strike": return "Strikethrough";
            case "sz": return `Size: ${globalXmlParser.attr(prop, "val")}`;
            case "color": return `Color: ${globalXmlParser.attr(prop, "val")}`;
            case "rFonts": return `Font: ${globalXmlParser.attr(prop, "ascii") || globalXmlParser.attr(prop, "hAnsi")}`;
            case "highlight": return `Highlight: ${globalXmlParser.attr(prop, "val")}`;
            case "jc": return `Align: ${globalXmlParser.attr(prop, "val")}`;
...
class HtmlRenderer {
    constructor(htmlDocument) {
        ...
        this.commentMap = {};
        this.trackChangeMap = {};
        this.trackChangeHighlights = null;
        this.currentMarginContainer = null;
    ...
    async render(document, bodyContainer, styleContainer = null, options) {
        ...
        if (this.trackChangeHighlights && ((options.renderChanges && (options.trackChangesMode === 'margin' || options.trackChangesMode === 'floating'))
            ...
            CSS.highlights.set(`${this.className}-tc-move-to`, this.trackChangeHighlights.moveTo);
...
`;
    ...
    renderCommentWithMargin(comment) {
    ...
    renderTrackChangeInline(elem, changeType) {
        const typeLabels = {
            'inserted': 'Inserted',
            'deleted': 'Deleted',
            'moveFrom': 'Moved from',
    ...
    renderTrackChangeWithMargin(elem, changeType) {
    ...
    extractPreviewText(elem, maxLength = 50) {
        let text = '';
